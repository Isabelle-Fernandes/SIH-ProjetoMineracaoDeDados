...
process_sih_rj <- function(data, information_system = "SIH-RJ", municipality_data = TRUE) {
  # Check information system
  available_information_system <- "SIH-RJ"
  ...
  if(information_system == "SIH-RJ"){
    ...
    #SITUACAO REJEICAO
    if("ST_SITUAC" %in% variables_names){
      data <- data %>%
        dplyr::mutate(ST_SITUAC = as.character(.data$ST_SITUAC)) %>%
        dplyr::mutate(ST_SITUAC = dplyr::case_match(
          .data$ST_SITUAC,
          "0" ~ "AIH autorizada",
          "1" ~ "AIH rejeitada",
          .default = .data$ST_SITUAC
        )) %>%
        dplyr::mutate(ST_SITUAC = as.factor(.data$ST_SITUAC))
    }
    
    #SITUACAO BLOQUEIO
    if("ST_BLOQ" %in% variables_names){
      data <- data %>%
        dplyr::mutate(ST_BLOQ = as.character(.data$ST_BLOQ)) %>%
        dplyr::mutate(ST_BLOQ = dplyr::case_match(
          .data$ST_BLOQ,
          "0" ~ "AIH autorizada",
          "2" ~ "Bloqueio AIH",
          "4" ~ "Cancelamento AIH",
          "5" ~ "Liberação bloqueio AIH",
          .default = .data$ST_BLOQ
        )) %>%
        dplyr::mutate(ST_BLOQ = as.factor(.data$ST_BLOQ))
    }
...
process_sih_er <- function(data, information_system = "SIH-ER", municipality_data = TRUE) {
  # Check information system
  available_information_system <- "SIH-ER"
  if(!(information_system %in% available_information_system)) stop("Health informaton system unknown.")
  ...
  if(information_system == "SIH-ER"){
    
    # MUNIC_RES
    if("MUN_RES" %in% variables_names & municipality_data == TRUE){
      colnames(tabMun)[1] <- "MUN_RES"
      data <- data %>%
        dplyr::mutate(MUN_RES = as.numeric(.data$MUN_RES)) %>%
        dplyr::left_join(tabMun, by = "MUN_RES")
    }
